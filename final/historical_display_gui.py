...
def get_font_path():
    ...
    return os.path.join(base_path, "IskraCYR-BoldItalic.otf")

def get_mascot_path(filename):
    if getattr(sys, 'frozen', False):
        base_path = sys._MEIPASS
    else:
        base_path = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(base_path, "Маскот", filename)
...
class HistoricalDisplayApp:
    ...
    def load_font(self):
        ...
        font_path_alt = "arial.ttf"
        for font_name in [font_path, font_path_alt]:
            try:
    ...
    def create_ui(self):
        ...
        # Main container using PanedWindow for 2:3 ratio (draggable)
        main_frame = tk.PanedWindow(self.bg_canvas, bg="#23363d", orient=tk.HORIZONTAL, sashwidth=5)
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        
        # Left side - controls (2 parts ≈ 40%)
        left_frame = tk.Frame(main_frame, bg="#23363d")
        main_frame.add(left_frame, minsize=350, stretch='never')
        ...
        tk.Label(date_inner, text="День:", fg="white", bg="#23363d", font=("Arial", 12, "bold")).grid(row=0, column=0, padx=5)
        ...
        tk.Label(date_inner, text="Месяц:", fg="white", bg="#23363d", font=("Arial", 12)).grid(row=0, column=2, padx=1)
        ...
        # Side controls
        for i in range(4):
            side_frame = tk.LabelFrame(left_frame, text=f"Сторона {i+1}", font=("Arial", 11, "bold"),
                                       fg="white", bg="#23363d", bd=1, relief=tk.GROOVE, padx=5, pady=3)
            side_frame.pack(fill=tk.X, pady=2)
            self.create_side_controls_wide(side_frame, i)
        
        # Folder and buttons
        folder_frame = tk.LabelFrame(left_frame, text="Сохранение", font=("Arial", 11, "bold"),
                                     fg="white", bg="#23363d", bd=1, relief=tk.GROOVE, padx=5, pady=3)
        folder_frame.pack(fill=tk.X, pady=2)
        
        self.save_folder_var = tk.StringVar(value=os.getcwd())
        tk.Entry(folder_frame, textvariable=self.save_folder_var, width=30, font=("Arial", 12)).pack(side=tk.LEFT, padx=2)
        ...
        tk.Button(btn_inner, text="Собрать", command=self.generate_image, bg="#2d8f4e", fg="white", font=("Arial", 12, "bold"), width=8).pack(side=tk.LEFT, padx=2)
        tk.Button(btn_inner, text="Сохранить", command=self.save_image, bg="#4a6fa5", fg="white", font=("Arial", 12)).pack(side=tk.LEFT, padx=2)
        
        # Right side - 4 previews in row (3 parts ≈ 60%)
        right_frame = tk.Frame(main_frame, bg="#23363d")
        main_frame.add(right_frame, minsize=600)
        
        # Use grid for 4 equal columns
        right_frame.grid_rowconfigure(0, weight=1)
        for c in range(4):
            right_frame.grid_columnconfigure(c, weight=1, uniform="col")
        
        # 4 preview canvases in a row
        for i in range(4):
            preview_frame = tk.LabelFrame(right_frame, text=f"Сторона {i+1}", font=("Arial", 10),
                                         fg="white", bg="#23363d", bd=1, relief=tk.GROOVE)
            preview_frame.grid(row=0, column=i, sticky="nsew", padx=2, pady=2)
            
            self.sides_data[i]['mini_canvas'] = tk.Canvas(preview_frame, bg="#23363d", highlightthickness=0)
            self.sides_data[i]['mini_canvas'].pack(fill=tk.BOTH, expand=True)
    ...
    def create_side_controls_wide(self, parent, side_num):
        ...
        tk.Label(year_frame, text="Год:", fg="white", bg="#23363d", font=("Arial", 12), width=4).pack(side=tk.LEFT)
        self.sides_data[side_num]['year_var'] = tk.StringVar()
        tk.Entry(year_frame, textvariable=self.sides_data[side_num]['year_var'], width=5, font=("Arial", 12)).pack(side=tk.LEFT, padx=2)
        
        # Event - wider dropdown
        event_frame = tk.Frame(parent, bg="#23363d")
        event_frame.pack(fill=tk.X, pady=1)
        tk.Label(event_frame, text="Событие:", fg="white", bg="#23363d", font=("Arial", 12), width=7).pack(side=tk.LEFT)
        ...
        tk.Label(img_frame, text="Картинка:", fg="white", bg="#23363d", font=("Arial", 12), width=7).pack(side=tk.LEFT)
        ...
        tk.Button(img_frame, text="Загр", command=lambda s=side_num: self.load_images_async(s), bg="#4a6fa5", fg="white", font=("Arial", 14), width=4).pack(side=tk.LEFT, padx=2)
        
        
        
        # Status label
        self.sides_data[side_num]['status_label'] = tk.Label(img_frame, text="", fg="#aaaaaa", bg="#23363d", font=("Arial", 14))
        self.sides_data[side_num]['status_label'].pack(side=tk.LEFT, padx=2)
        
        # Mascot is automatically assigned per side (1,2,3,4 automatically)

    def on_year_text_edit(self, side_num):
        """Handle manual edit of year text in mini preview."""
        year_widget = self.sides_data[side_num].get('year_text_widget')
        if year_widget:
            edited_year = year_widget.get("1.0", tk.END).strip()
            self.sides_data[side_num]['edited_year'] = edited_year
            year_widget.edit_modified(False)
            # Also update the year_var
            import re
            year_match = re.search(r'\d{4}', edited_year)
            if year_match:
                self.sides_data[side_num]['year_var'].set(year_match.group())
    ...
    def create_side_controls_compact(self, parent, side_num):
        ...
        tk.Label(year_frame, text="Год:", fg="white", bg="#23363d", font=("Arial", 12), width=4).pack(side=tk.LEFT)
        self.sides_data[side_num]['year_var'] = tk.StringVar()
        tk.Entry(year_frame, textvariable=self.sides_data[side_num]['year_var'], width=6, font=("Arial", 12)).pack(side=tk.LEFT, padx=2)
        
        # Event
        event_frame = tk.Frame(parent, bg="#23363d")
        event_frame.pack(fill=tk.X, pady=1)
        tk.Label(event_frame, text="Соб:", fg="white", bg="#23363d", font=("Arial", 12), width=4).pack(side=tk.LEFT)
        self.sides_data[side_num]['event_var'] = tk.StringVar()
        self.sides_data[side_num]['event_combo'] = ttk.Combobox(event_frame, 
                                                                  textvariable=self.sides_data[side_num]['event_var'],
        ...
        tk.Entry(event_frame, textvariable=self.sides_data[side_num]['manual_event_var'], width=15, font=("Arial", 12)).pack(side=tk.LEFT, padx=2)
        
        # Image with progress bar
        img_frame = tk.Frame(parent, bg="#23363d")
        img_frame.pack(fill=tk.X, pady=1)
        tk.Label(img_frame, text="Кар:", fg="white", bg="#23363d", font=("Arial", 12), width=4).pack(side=tk.LEFT)
        self.sides_data[side_num]['image_var'] = tk.StringVar()
        self.sides_data[side_num]['image_combo'] = ttk.Combobox(img_frame,
                                                                  textvariable=self.sides_data[side_num]['image_var'],
                                                                  state='readonly', width=18, font=("Arial", 12))
        ...
        tk.Button(img_frame, text="Загр", command=lambda s=side_num: self.load_images_async(s), bg="#4a6fa5", fg="white", font=("Arial", 14), width=5).pack(side=tk.LEFT, padx=2)
    ...
    def on_image_select(self, side_num):
        ...
        try:
            ...
            if idx > 0:  # Has image selected
                ...
                # Check if "Своя картинка" is selected
                image_list = self.sides_data[side_num]['image_combo']['values']
                if idx == len(image_list) - 1:  # Last item is "Своя картинка"
                    self.select_custom_image(side_num)
                    return
                ...
                if images and idx-1 < len(images):
                    try:
                        ...
                        if img_obj:
                            ...
                            self.sides_data[side_num]['custom_image'] = None
    ...
    def update_mini_preview(self, side_num, event_text):
        ...
        # Get actual canvas size (dynamic) - use minimum
        width = max(canvas.winfo_width() or 100, 100)
        height = max(canvas.winfo_height() or 200, 200)
        
        # Use fixed proportions like final image (520x1872 ≈ 1:3.6)
        # Scale to fit width, then use that height
        if width < 50:
            width = 100
        if height < 50:
            height = 200
        scale = width / 130  # Scale based on width
        ...
        # Draw date (top ~10%)
        date_y = int(30 * scale)
        canvas.create_text(width//2, date_y, text=f"{self.day_var.get()} {RUSSIAN_MONTHS.get(int(self.month_var.get()), 'янв')[:3]}", fill='white', font=('Arial', int(13*scale), 'bold'), justify='center')
        
        # Draw subtitle (~5%)
        canvas.create_text(width//2, date_y + int(20*scale), text="этот день в истории", fill='white', font=('Arial', int(11*scale)))
        
        # Separator
        sep1_y = date_y + int(40*scale)
        canvas.create_line(int(5*scale), sep1_y, width-int(5*scale), sep1_y, fill='white', width=int(4*scale))
        
        # Draw year (editable) - ~10%
        year = self.sides_data[side_num]['year_var'].get()
        year_text = f"В {year} году" if year and str(year).strip() else "В ???? году"
        
        year_area_top = sep1_y + int(5*scale)
        year_area_height = int(30*scale)
        
        if 'year_text_widget' not in self.sides_data[side_num]:
            year_widget = tk.Text(canvas, width=15, height=1, font=('Arial', int(12*scale), 'bold'), bg='#1a2a30', fg='white', bd=0)
            year_widget.place(x=int(30*scale), y=year_area_top, width=width-int(60*scale), height=year_area_height)
            self.sides_data[side_num]['year_text_widget'] = year_widget
            year_widget.insert("1.0", year_text)
            year_widget.bind('<<Modified>>', lambda e, s=side_num: self.on_year_text_edit(s))
        else:
            year_widget = self.sides_data[side_num]['year_text_widget']
            year_widget.configure(font=('Arial', int(12*scale), 'bold'))
            year_widget.delete("1.0", tk.END)
            year_widget.place_configure(x=int(30*scale), y=year_area_top, width=width-int(60*scale), height=year_area_height)
            year_widget.insert("1.0", year_text)
        
        # Separator
        sep2_y = year_area_top + year_area_height + int(5*scale)
        canvas.create_line(int(5*scale), sep2_y, width-int(5*scale), sep2_y, fill='white', width=int(4*scale))
        
        # Event text area (editable) - middle ~35%
        text_area_top = sep2_y + int(5*scale)
        text_area_height = int(130*scale)
        canvas.create_rectangle(int(5*scale), text_area_top, width-int(5*scale), text_area_top + text_area_height, fill='#1a2a30', outline='white', width=int(4*scale))
        
        if 'event_text_widget' not in self.sides_data[side_num]:
            text_widget = tk.Text(canvas, width=32, height=3, font=('Arial', int(10*scale)), bg='#1a2a30', fg='white', bd=0)
            text_widget.place(x=int(10*scale), y=text_area_top + int(5*scale), width=width-int(20*scale), height=text_area_height-int(10*scale))
            ...
            text_widget.configure(font=('Arial', int(10*scale)))
            text_widget.delete("1.0", tk.END)
            text_widget.place_configure(x=int(10*scale), y=text_area_top + int(5*scale), width=width-int(20*scale), height=text_area_height-int(10*scale))
            text_widget.insert("1.0", event_text)
        
        # Image area (below text) - bottom ~25%
        img_area_top = text_area_top + text_area_height + int(10*scale)
        canvas.create_text(width//2, img_area_top + int(30*scale), text="[Картинка]", fill='#888888', font=('Arial', int(10*scale)))
        
        # Mascot area - bottom right ~15%
        mascot_size = int(40*scale)
        mascot_x = width - mascot_size - int(10*scale)
        mascot_y = height - mascot_size - int(10*scale)
        canvas.create_rectangle(mascot_x, mascot_y, width-int(5*scale), height-int(5*scale), outline='#666666', width=1)
        canvas.create_text(mascot_x + mascot_size//2, mascot_y + mascot_size//2, text="Маскот", fill='#666666', font=('Arial', int(8*scale)))
    ...
    def update_mini_preview_with_image(self, side_num, event_text, img_obj):
        ...
        # Get actual canvas size (dynamic)
        width = canvas.winfo_width() or 200
        height = canvas.winfo_height() or 400
        
        # Scale factor based on original 260x310
        scale = min(width / 260, height / 310)
        ...
        date_y = int(30 * scale)
        canvas.create_text(width//2, date_y, text=f"{self.day_var.get()} {RUSSIAN_MONTHS.get(int(self.month_var.get()), 'янв')[:3]}", fill='white', font=('Arial', int(13*scale), 'bold'), justify='center')
        canvas.create_text(width//2, date_y + int(18*scale), text="Этот день", fill='white', font=('Arial', int(11*scale)))
        canvas.create_text(width//2, date_y + int(34*scale), text="в истории", fill='white', font=('Arial', int(11*scale)))
        
        sep1_y = date_y + int(50*scale)
        canvas.create_line(int(5*scale), sep1_y, width-int(5*scale), sep1_y, fill='white', width=int(4*scale))
        ...
        # Event text area (editable) - middle ~25%
        text_area_top = sep2_y + int(5*scale)
        text_area_height = int(80*scale)
        canvas.create_rectangle(int(5*scale), text_area_top, width-int(5*scale), text_area_top + text_area_height, fill='#1a2a30', outline='white', width=int(4*scale))
        ...
        # Image (in bottom area)
        img_area_top = text_area_top + text_area_height + int(10*scale)
        img_max_height = height - img_area_top - int(50*scale)
        
        if img_obj:
            img_obj_copy = img_obj.copy()
            img_obj_copy.thumbnail((width-int(20*scale), img_max_height), Image.Resampling.LANCZOS)
            try:
                photo = ImageTk.PhotoImage(img_obj_copy)
                canvas.create_image(width//2, img_area_top + img_max_height//2, image=photo, anchor=tk.CENTER, tags=('img',))
                canvas.image = photo
                ...
                canvas.create_text(width//2, img_area_top + img_max_height//2, text="[Изображение]", fill='#888888', font=('Arial', int(10*scale)))
        ...
        mascot_y = height - mascot_size - int(10*scale)
        canvas.create_rectangle(mascot_x, mascot_y, width-int(5*scale), height-int(5*scale), outline='#666666', width=1)
        canvas.create_text(mascot_x + mascot_size//2, mascot_y + mascot_size//2, text="М", fill='#666666', font=('Arial', int(8*scale)))
    ...
    def _update_images_ready(self, side_num, images):
        ...
        try:
            ...
            image_list.append("Своя картинка")
    ...
    def calculate_event_font_size(self, all_texts):
        ...
        padding = 10
        block_width = 500
        block_height = 340 - padding * 2
        
        # Start from larger font size for bigger text
        for size in range(120, 20, -2):
            try:
                test_font = ImageFont.truetype(get_font_path(), size)
            except:
                try:
            ...
            for text in all_texts:
                ...
                chars_per_line = max(8, int(block_width / (size * 0.55)))
    ...
    def create_side_image(self, month, day, side_num, event_font_size=42):
        ...
        date_text = f"{day} {RUSSIAN_MONTHS.get(month, 'января')}"
        ...
        # Use edited year from preview if available, otherwise from year_var
        edited_year = self.sides_data[side_num].get('edited_year', '')
        display_year = edited_year if edited_year else year
        # Extract just the year number if full text was edited
        if display_year:
            import re
            year_match = re.search(r'\d{4}', str(display_year))
            if year_match:
                display_year = year_match.group()
        year_text = f"В {display_year} году" if display_year and str(display_year).strip() else "В ??? году"
        ...
        if event_text:
            ...
            try:
                event_font = ImageFont.truetype(get_font_path(), event_font_size)
            except:
                try:
    ...
    def load_mascot(self, side_number):
        """Load mascot image."""
        # side_number is 1-4, convert to 0-3 for array indexing
        side_idx = side_number - 1
        if side_idx < 0 or side_idx > 3:
            return None
        
        # Check for custom mascot first
        custom = self.sides_data[side_idx].get('custom_mascot')
        if custom:
            return custom
        ...
        try:
            mascot_img = Image.open(get_mascot_path(mascot_files[side_number]))
    ...
    def save_image(self):
        ...
        # Save image (always regenerate to be safe)
        try:
            # Get selected date
            try:
                day = int(day)
    ...
    def select_custom_image(self, side_num):
        """Select custom image for the side."""
        filename = filedialog.askopenfilename(
            title=f"Выберите картинку для стороны {side_num}",
            filetypes=[("PNG images", "*.png"), ("JPG images", "*.jpg"), ("All files", "*.*")]
        )
        if filename:
            try:
                img = Image.open(filename)
                self.sides_data[side_num]['custom_image'] = img
                self.sides_data[side_num]['image'] = img
                self.refresh_mini_preview(side_num)
            except Exception as e:
                messagebox.showerror("Ошибка", f"Не удалось загрузить изображение: {e}")
    
    def select_custom_mascot(self, side_num):
        """Select custom mascot image."""
        filename = filedialog.askopenfilename(
            title=f"Выберите маскот для стороны {side_num}",
            filetypes=[("PNG images", "*.png"), ("JPG images", "*.jpg"), ("All files", "*.*")]
        )
        if filename:
            try:
                ...
                self.sides_data[side_num]['custom_mascot'] = img
                self.sides_data[side_num]['mascot_var'].set("Свой")
                basename = os.path.basename(filename)
                self.sides_data[side_num]['mascot_label'].config(text=basename[:15])
            except Exception as e:
                messagebox.showerror("Ошибка", f"Не удалось загрузить изображение: {e}")
